...
DotnetGenerator::DotnetGenerator(InterfaceDefinition *def, const std::string dotnetPackageName) :
Generator(def, generator_type_t::kDotnet), m_dotnetPackageName(dotnetPackageName), m_suffixStrip(""), m_suffixStripSize(0)
{
    /* Set copyright rules. */
...
void DotnetGenerator::generateOutputFiles(const string &fileName)
{
...
void DotnetGenerator::generateCommonFile(string fileName)
{
...
void DotnetGenerator::generateConstFile(string fileName)
{
    fileName += "/common/";
    ...
    std::string classFilename = fileName + "/Constants.cs";
    generateOutputFile(classFilename, "dotnet_const", m_templateData, kDotnetConst);
}

void DotnetGenerator::generateClientFile(string fileName)
{
    ...
    for (data_ptr interface : list)
    {
        ...
        std::string classFilename = fileName + "/" + name + "Client.cs";
        generateOutputFile(classFilename, "dotnet_client", m_templateData, kDotnetClient);
    }
}

void DotnetGenerator::generateServerFile(string fileName)
{
    data_map &map = m_templateData["group"].get().get()->getmap();
    data_list &list = map["interfaces"]->getlist();
    fileName += "/server";
    ...
    for (data_ptr interface : list)
    {
        std::string name = interface.get().get()->getmap()["name"].get().get()->getvalue();
        m_templateData["interface"] = interface;
        std::string classFilename = fileName + "/Abstract" + name + "Service.cs";
        generateOutputFile(classFilename, "dotnet_server", m_templateData, kDotnetServer);
    }
}

void DotnetGenerator::generateInterfaceFile(string fileName)
{
    data_map &map = m_templateData["group"].get().get()->getmap();
    data_list &list = map["interfaces"]->getlist();
    fileName += "/interfaces";
    ...
    for (data_ptr interface : list)
    {
        std::string name = interface.get().get()->getmap()["name"].get().get()->getvalue();
        m_templateData["interface"] = interface;
        std::string classFilename = fileName + "/I" + name + ".cs";
        generateOutputFile(classFilename, "dotnet_interface", m_templateData, kDotnetInterface);
    }
}

void erpcgen::DotnetGenerator::generateEnumFiles(std::string fileName)
{
    ...
    for (data_ptr item : list)
    {
        ...
        std::string classFilename = fileName + "/" + name + ".cs";
        generateOutputFile(classFilename, "dotnet_enum", m_templateData, kDotnetEnum);
    }
}

void erpcgen::DotnetGenerator::generateStructFiles(std::string fileName)
{
    ...
    for (data_ptr item : list)
    {
        ...
        // dump_data(item);
        std::string classFilename = fileName + "/" + name + ".cs";
        generateOutputFile(classFilename, "dotnet_struct", m_templateData, kDotnetStruct);
    }
}

void DotnetGenerator::parseSubtemplates()
{
    const char *templateName = "dotnet_coders";
    try
    {
        parse(kDotnetCoders, m_templateData);
    }
...
void DotnetGenerator::generate()
{
    ...
    m_templateData["appPackage"] = make_data(m_dotnetPackageName);
...
void DotnetGenerator::setTemplateComments(Symbol *symbol, data_map &symbolInfo)
{
    symbolInfo["mlComment"] = symbol->getMlComment();
    symbolInfo["ilComment"] = symbol->getIlComment();
}

data_map DotnetGenerator::getFunctionTemplateData(Group *group, Function *fn)
{
    (void)group;
    data_map info;
...
string DotnetGenerator::getFunctionPrototype(Group *group, FunctionBase *fn, const string &interfaceName,
...
string DotnetGenerator::getTypenameName(DataType *t, bool isReference, bool objectType)
{
    string returnName;
...
string DotnetGenerator::getArrayInitialization(ArrayType *t)
{
    string result = "";
    ...
    {
        result = "List" + result; // Dotnet does not support typed arrays e.g. List<int[]>. Use untyped List[].
    }
    else
    {
        result = getTypenameName(t->getTrueContainerDataType(), false, false) + result;
...
void DotnetGenerator::makeConstTemplateData()
{
...
void DotnetGenerator::makeEnumsTemplateData()
{
    Log::info("Enums:\n");
    data_list enums;
    int n = 0;
...
data_list DotnetGenerator::getEnumMembersTemplateData(EnumType *enumType)
{
    int j = 0;
...
void DotnetGenerator::makeAliasesTemplateData()
{
...
data_map DotnetGenerator::makeGroupSymbolsTemplateData(Group *group)
{
...
void DotnetGenerator::setStructMembersTemplateData(StructType *structType, data_map &structInfo)
{
    data_list members;
...
void DotnetGenerator::setOneStructMemberTemplateData(StructMember *member, data_map &member_info)
{
...
data_map DotnetGenerator::getEncodeDecodeCall(const string &name, DataType *t, StructType *structType,
                                            ...
                                            bool needTypeDeclaration, bool isFunctionParam)
{
    ...
    switch (t->getDataType())
    {
        ...
        {
            throw internal_error("Dotnet does not support functions yet.");
        }
        case DataType::data_type_t::kListType:
        {
...
string DotnetGenerator::capitalize(string text)
{
    text[0] = toupper(text[0]);
    return text;
}

void DotnetGenerator::makeFunctionsTemplateData()
{
...
data_map DotnetGenerator::getTypeInfo(DataType *t, int isReference, bool inDataContainer)
{
    data_map info;
    std::string name = filterName(getOutputName(t, false));
    info["name"] = name;
    info["typeName"] = getTypenameName(t, isReference, isReference); // Dotnet type: Float, List<int[]>, ...
...
string DotnetGenerator::getBuiltinCodecTypeName(const BuiltinType *t)
{
...
string DotnetGenerator::getBuiltinTypename(const BuiltinType *t)
{
    switch (t->getBuiltinType())
    {
        case BuiltinType::builtin_type_t::kBoolType:
...
string DotnetGenerator::getBuiltinObjectTypename(const BuiltinType *t)
{
    switch (t->getBuiltinType())
    {
        case BuiltinType::builtin_type_t::kBoolType:
...
string DotnetGenerator::filterName(const string &name)
{
    string result = name;
...
std::string erpcgen::DotnetGenerator::getGroupPackageName(const Group *group)
{
    string name = m_dotnetPackageName;
...
bool DotnetGenerator::checkWhitespaceChar(char c)
{
    if (c == ' ' || c == '\t' || c == '\n' || c == '\r')
    {
        return true;
...
string DotnetGenerator::stripWhitespace(const string &s)
{
    string result = s;
    int32_t i;
    uint32_t n;
...
void DotnetGenerator::initDotnetReservedWords()
{

    // Dotnet reserved words
